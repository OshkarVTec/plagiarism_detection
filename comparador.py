...
def run_deckard_detector(dataset_path):
    ...
    clones = deckard.run_deckard_on_directory(dataset_path,
                                             min_size=5,
                                             window_size=3,
                                             min_dist=5.0,
                                             k=5, L=10, w=4.0)
...
def main():
    ...
    deckard_df, deckard_time, deckard_mem = run_with_profiler(run_deckard_detector, dataset_path)
    print(f"Deckard tiempo: {deckard_time:.2f} s, pico memoria: {deckard_mem:.2f} MB")

    # Ejecutar Nuestro
    print("Ejecutando Nuestro...")
    nuestro_df, nuestro_time, nuestro_mem = run_with_profiler(run_nuestro_detector, dataset_path)
